/* 
MED-004: Database Index Optimization Migration
...
CREATE INDEX IF NOT EXISTS idx_transactions_user_uncertain ON transactions(user_id, is_uncertain) WHERE is_uncertain = TRUE;
...
CREATE INDEX IF NOT EXISTS idx_transactions_desc_search ON transactions USING gin(to_tsvector('english', description_norm || ' ' || COALESCE(description_raw, '')));
